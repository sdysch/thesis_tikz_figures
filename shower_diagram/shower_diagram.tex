\begin{tikzpicture}[scale=0.9]
	...
	\draw[thick, color = black] (\legXmin - 2, \legYmin) -- (\legXmax + 1.5, \legYmin) -- (\legXmax + 1.5, \legYmax);
	...
	\node at (\legXmin, \keyY - 2.5) {Hadronisation};
	\node at (\legXmin, \keyY - 3.0) {and particle decay};
	\draw[hadronisation = {\keyX, \keyY - 1.5, 180}];
	\draw[hadronisation2 = {\keyX, \keyY - 2.5, 180}];
	\draw[hadronisationSmallDecay = {\keyX, \keyY - 3.5, 180}];

	%\draw [decorate, decoration = {brace, mirror, amplitude = 10pt}, xshift = 0, yshift = 0] (\keyX - 0.5, \keyY - 2.5) -- (\keyX - 0.5, \keyY - 3.5) node [black, midway, xshift = 0] {};


	\node at (\legXmin, \keyY - 4.5) {Underlying event};
	\draw[smallGluon, color = orange] (\keyX, \keyY - 4.4) -- (\keyX + 1, \keyY - 4.4);
	\draw[thick, color = orange] (\keyX, \keyY - 4.6) -- (\keyX + 1, \keyY - 4.6);
	\draw[color = black, thick, fill = orange] (\keyX, \keyY - 4.5) circle [x radius = 0.5 * \smallEllipseRx, y radius = 0.5 * \smallEllipseRy];

	\node at (\legXmin, \keyY - 5.5) {Proton scattering};
	\draw[color = blue, thick] (\keyX + 0.2, \keyY - 5.4) -- (\keyX + 1, \keyY - 5.4);
	\draw[smallGluon, color = blue] (\keyX + 0.2, \keyY - 5.6) -- (\keyX + 1, \keyY - 5.6);
	\draw[protonScatter = {\keyX + 1, \keyY - 5.5, \verySmallEllipseRy}];

	\node at (\legXmin, \keyY - 0.5 - 6.3) {Parton shower};
	\draw[smallGluon, color = magenta] (\keyX, \keyY - 0.5 - 6.4) -- (\keyX + 1, \keyY - 0.5 - 6.4);
	\draw[thick, color = magenta] (\keyX, \keyY - 0.5 - 6.6) -- (\keyX + 1, \keyY - 0.5 - 6.6);
	\draw[color = black, fill = magenta, thick] (\keyX, \keyY - 0.5 - 6.0) circle (0.5 * \hardScatterRadius);
